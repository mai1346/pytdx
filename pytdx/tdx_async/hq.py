...
from pytdx.tdx_async.reflection_async import make_async_parser
...
class ATdxHq_API:
    ...
    async def get_security_bars(self, category: int, market: int, code: str, 
                              start: int, count: int, connection: Optional['AsyncTrafficStatSocket'] = None) -> pd.DataFrame:
    ...
    async def get_security_quotes(self, all_stock: Union[Tuple[int, str], List[Tuple[int, str]]], 
                               code: Optional[str] = None, connection: Optional['AsyncTrafficStatSocket'] = None) -> pd.DataFrame:
        if code is not None:
    ...
    async def get_history_transaction_data(self, market: int, code: str, start: int, 
                                         count: int, date: int, connection: Optional['AsyncTrafficStatSocket'] = None) -> pd.DataFrame:
    ...
    async def get_company_info_content(self, market: int, code: str, filename: str, 
                                     start: int, length: int, connection: Optional['AsyncTrafficStatSocket'] = None) -> pd.DataFrame:
    ...
    def get_and_parse_block_info(self, blockfile: str) -> pd.DataFrame:
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        try:
            res = [api.get_security_bars(8, 0, '000001', 0, 800) for _ in range(10)]
